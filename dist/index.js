module.exports = function(e, t) {
  ...
  543 : function(e, t, n) {
    ...
    const i = async e => {
      ...
      let n = "";
      try {
        n = await s("which sudo")
      } catch (e) {
      }
      console.log("Uninstalling previous CML");
      await s(
          `${n} npm uninstall -g canvas vega vega-cli vega-lite @dvcorg/cml`);
      console.log(`Installing CML version ${t}`);
      await s("npm config set user 0");
      await s(`${n} npm install -g canvas vega vega-cli vega-lite @dvcorg/cml${
          t !== "latest" ? `@${t}` : ""}`)
    };
    t.exec = s;
    t.setup_cml = i
